...
const Services = (props) => {
  ...
  return (
   ...
   <h1 className='text-center text-3xl font-bold' style={{color: props.webMode === 'light'? 'black': 'white'}}>My Services</h1>
    <div className='p-24 grid grid-cols-3 gap-x-3 m-16 bg-gray-200 rounded-3xl sm:grid-cols-1 sm:p-8 sm:gap-y-6 sm:m-8 md:grid-cols-1 md:gap-y-6 lg:grid-cols-3'>
    {services.map(({id, image, Title, Description}) => {
        return(

    <div className=' p-3 text-center rounded-3xl h-60 mx-6 hover:translate-y-[-10px] card-section sm:mx-3 ' key={id}>
